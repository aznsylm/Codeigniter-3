...
class Berita extends CI_Controller 
{
    ...
    public function index()
    {
        ...
        $data = array('judul' => "DATA BERITA",'sub' => "Lihat Berita",'read'=> $q);
        $this->template->load('backend/template',$this->view.'read', $data);
    }

    public function create()
    {
        $data = array('judul' => "DATA BERITA",'sub' => "Tambah Berita",'kategori' => $this->M_kategori->GetAll(),'create'=>'');
    ...
    {
        $kd = $this->uri->segment(3);
        $data = array('st_berita' => $this->uri->segment(4));
        $this->M_berita->update($kd,$data);
        redirect($this->redirect,'refresh');
    }
